...
table_by_group <- function(df, x){
  ...
  colnames(table) <- to_any_case(colnames(table), 'title') # rename the group column 
  colnames(table)[1] <- to_any_case(x, 'title')
  return(table)
}

# table_by_group(coffee_ratings_cleaned, "binned_altitude_mean_meters")
...
groups <- c('species', 'country_of_origin', 'binned_altitude_mean_meters')
output <- map(groups, ~table_by_group(coffee_ratings_cleaned, .x))
knitr::kable(output, 'simple')
```
...
scatter_plot <- function(df, group, varx, vary){
  ...
  df <- df %>%
    ...
    rename(varx = all_of(varx)) %>%
    rename(vary = all_of(vary)) %>% 
